...
function FreeShippingIllustration() {
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      <svg width="100%" height="100%" viewBox="0 0 260 160" fill="none" preserveAspectRatio="xMidYMid meet">
  ...
  return (
    <div className="relative w-full overflow-hidden"
      ...
      <div className="flex transition-transform duration-700 ease-in-out items-stretch"
        ...
        <div className="w-full flex-shrink-0">
          ...
          >
            ...
            <div className="relative text-center px-6 max-w-3xl" style={{ zIndex: 3 }}>
              ...
              <Link
                href="/collections"
                className="inline-block font-display text-base px-8 py-4 rounded-full shadow-md hover:scale-105 transition-transform"
                style={{ background: '#1f3a52', color: '#ffffff' }}
              >
                Start shopping
              </Link>
        ...
        <div className="w-full flex-shrink-0">
          <section
            ...
            style={{ background: 'linear-gradient(180deg, #eef5fb 0%, #dbe9f5 100%)' }}
          >
            <div className="relative w-[64%] sm:w-[46%] md:w-[36%] max-w-xs h-[26vh] md:h-[32vh] flex items-center justify-center" style={{ zIndex: 2 }}>
              <BigBearIllustration />
            </div>
            ...
            <div className="relative text-center px-6 max-w-3xl" style={{ zIndex: 3 }}>
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                <Link
                  ...
                  className="inline-flex items-center gap-2 rounded-full font-display text-base px-8 py-4 shadow-md hover:scale-105 transition-transform"
                  style={{ background: '#1f3a52', color: '#ffffff' }}
                >
      ...
      <button type="button" onClick={() => go(index - 1)} aria-label="Previous slide"
              className="absolute left-3 md:left-6 top-1/2 -translate-y-1/2 z-30 w-11 h-11 md:w-14 md:h-14 rounded-full bg-white/80 backdrop-blur-sm hover:bg-white text-charcoal shadow-lg flex items-center justify-center transition-all">
        <ChevronIcon direction="left" />
      </button>
      <button type="button" onClick={() => go(index + 1)} aria-label="Next slide"
              className="absolute right-3 md:right-6 top-1/2 -translate-y-1/2 z-30 w-11 h-11 md:w-14 md:h-14 rounded-full bg-white/80 backdrop-blur-sm hover:bg-white text-charcoal shadow-lg flex items-center justify-center transition-all">
        <ChevronIcon direction="right" />
      </button>

      {/* Dots */}
      <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 flex gap-2.5 z-30">
        {[...Array(SLIDE_COUNT)].map((_, i) => (
          <button key={i} type="button" onClick={() => go(i)} aria-label={'Go to slide ' + (i + 1)}
                  className={'h-3 rounded-full transition-all shadow-sm ' + (index === i ? 'w-8 bg-coral' : 'w-3 bg-white/70 hover:bg-white')} />
        ))}
      </div>
    </div>
  )
