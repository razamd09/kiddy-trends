...
export default function SummerBanner({ discountCode: initialCode = 'SUMMER50', href = '/collections' }) {
    ...
    async function fetchCodes() {
      try {
        ...
        if (Array.isArray(data.codes) && data.codes.length > 0) {
          // prefer an explicitly enabled code, fallback to first
  ...
  return (
    <section role="region" aria-label="Summer Sale banner" className="relative w-full overflow-hidden bg-gradient-to-r from-yellow-400 via-orange-500 to-pink-500 text-white">
      <div className="max-w-7xl mx-auto px-6 py-12 md:py-20 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="text-center md:text-left">
          ...
          <div className="mt-4 inline-flex items-center gap-3">
            ...
            <span className="bg-white text-black px-3 py-1 rounded font-semibold tracking-wide">{code}</span>
          </div>
        </div>

        <a href={href} className="mt-4 md:mt-0 inline-flex items-center justify-center bg-white text-black font-semibold px-6 py-3 rounded-lg shadow-lg hover:scale-105 transition-transform" aria-label="Shop now — all collections">
