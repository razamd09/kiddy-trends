...
// Sends a server-side Purchase event to Meta's Conversions API. No-ops
// (logs and returns) if META_CONVERSIONS_API_ACCESS_TOKEN isn't configured
// yet, so checkout never fails because of this being unset.
//
// eventId must match the eventID passed to the client-side fbq('track',
// 'Purchase', ..., { eventID }) call for the same order, so Meta
