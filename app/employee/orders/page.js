...
export default function EmployeeOrdersPage() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            const res = await fetch('/api/admin/orders?page=1&status=pending', { cache: 'no-store' })
    ...
    }, [ready, lastOrderCount])

    useEffect(() => {
        if (ready && typeof Notification !== 'undefined' && Notification.permission === 'default') {
    ...
    async function fetchOrders() {
        setLoading(true)
        const res = await fetch('/api/admin/orders?page=' + page + '&status=' + filter, { cache: 'no-store' })
        const data = await res.json()
                                            ...
                                            <a key={i} href="#"
                                               onClick={(e) => {
                                                   ...
                                                   if (handle) {
                                                       ...
                                                       alert('No product link saved for this item. Please search it manually on the store.')
                                                   }
                                               }}
                                               className="flex items-center justify-between bg-white rounded-xl px-3 py-2 text-sm hover:shadow-md hover:border-coral transition-all border-2 border-transparent cursor-pointer">
                                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        {item.handle && <p className="text-[10px] text-coral font-semibold mt-0.5">✓ Click to view product →</p>}
                                                    </div>
                                                </div>
